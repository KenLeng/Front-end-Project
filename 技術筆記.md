...
## <a name='2'>Nuxt Lifecycle</a>
 ...
 # 只有Server端可使用
  ...
  |asyncData|<ul><li>Vue Instance未建立，無法使用</li><li>可回傳物件，會覆蓋data()</li></ul>|server/client端|
...
## <a name='g'>Vue Lifecycle</a>
   ...
   * parent vs child life cycle 
   ...
   ```javascript
   # keep-alive組件
   是vue在緩存組件，緩存組建而不銷毀，所以切換到緩存的組件頁時，不會跑整個生命週期，而是執行相應的activated、deactivated生命週期函數，非mounted、unmounted函數。
...
## <a name='m'>`ES5、ES6`</a>
 ...
 ## <a name='2'>`macroTask(宏任務)`、`microTask(微任務)`</a>
   * 先執行macrotask再進行microtask，以此類推。因microTask也是在主程序跑，所以有機會還是會卡住
     * 原理 - 每個macroTask裡會有個microTask quene，透過Event loop執行task
